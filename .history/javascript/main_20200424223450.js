...
d3.csv("data/co2-emission-1970-2012new.csv")
    .then(function(csvdata){
        ...
        const worldCarbon = Object.values(csvdata[217]).splice(2,41).map(v=>{
            return parseFloat(v);
        });
        ...
        var wrl = csvdata[217];
        console.log(wrl);
        var test = Object.entries(wrl).splice(2,43).map(key=>{
            return key, wrl[key];
        });
        console.log(test);
        Object.entries(wrl).forEach(([key, value]) => (`${key}: ${value}`)); 
