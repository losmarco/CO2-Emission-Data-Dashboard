...
d3.csv("data/co2-emission-1970-2012.csv")
    ...
    // const world = wrl.splice(4,43).map(([key, value]) => [wrl[key], parseFloat(value)]);

    year.forEach((number, index)=>{
        world[index][0] = number;
    });

    // var dataA = [[ "abc", "278.47" ],​[ "def", "241.3" ],​[ "ghi", "280.85" ]]
        
    // const newData = dataA.map(([key, value]) => [key, parseInt(value)]);
    ...
    //              .x(x(year))
    //              .y(y(valueNum));
    const check = [[1970,'78.47']​[1971,'41.3'][ 1972,'280.85']];
    var line = d3.line();
               // .x()
                //.y();
